...
```{r packages}
loadedPackages <- c("geojsonsf", "ggmap", "ggplot2", "graphics", "grDevices", "sf", "sparrpowR", "spatstat", "terra", "tidyterra")
...
dc <- geojson_sf(gis_path1)

# American Community Survey 2020 Census Tracts
gis_path2 <- "https://opendata.arcgis.com/datasets/95f1885eff9a4c089012a872c851c50a_8.geojson"
census <- geojson_sf(gis_path2)
```
...
```{r dc_owin}
clipwin <- st_union(census)
dcc <- st_intersection(dc, clipwin)
# Plot
plot(st_geometry(dc), main = "DC Boundary")
plot(st_geometry(census),  main = "American Community Survey\n2020")
plot(st_geometry(dcc), main = "Clipped Boundary")
```
...
dcp <- st_transform(st_as_sf(dcc), crs = st_crs(32618))
dco <- as.owin(st_geometry(dcp))
```
...
spf <- st_as_sf(navy, coords = c("lon", "lat"), crs = st_crs(32618))
# Plot
plot(st_geometry(dcp), main = "Location of Hypothetical\nDisease Cluster")
...
sim_power <- spatial_power(
  x_case = navy[[1]], y_case = navy[[2]], # center of cluster
...
spatial_plots(
  input = sim_power, # use output of above simulation
...
```{r basemap}
dcbb <- st_bbox(st_buffer(st_as_sf(dc), dist = 0.015))
dcbbm <- matrix(dcbb, nrow = 2)
base_map <- get_stadiamap(bbox = dcbbm, maptype = "stamen_terrain")
```
...
sim_pts <- st_as_sf(sim_pts) # convert to simple features
names(sim_pts)[1] <- "mark"
st_crs(sim_pts) <- st_crs(32618)
sim_pts_wgs84 <-  st_transform(sim_pts, crs = st_crs(4326)) # project to basemap
```
...
pvalprop <- data.frame(
  x = sim_power$rx,
  y = sim_power$ry,
  z = sim_power$pval_prop_cas
) # extract proportion significant
lrr_narm <- na.omit(pvalprop) # remove NAs
pvalprop_raster <- rast(lrr_narm) # convert to SpatRaster
rm(pvalprop, lrr_narm) # conserve memory
crs(pvalprop_raster) <- crs(dcp) # set output project (UTM 18N)
pvalprop_raster <- project(pvalprop_raster, dc) # unproject (WGS84)
rampcols <- colorRampPalette(colors = c(cols[1], cols[2]), space="Lab")(length(values(pvalprop_raster))) # set colorramp
```
...
ggmap(base_map) + # basemap
  geom_sf(
    data = dcc, # original boundary
    fill = "transparent",
    colour = "black",
    inherit.aes = FALSE
  ) +
  geom_spatraster(
    data = pvalprop_raster, # output SpatRaster
    size = 0,
    alpha = 0.5
  ) +
  scale_fill_gradientn(colours = rampcols, na.value = NA) + # colors for SpatRaster
  geom_sf(
    data = sim_pts_wgs84[-1, ], # simulated point-locations
    aes(color = mark, shape = mark),
    alpha = 0.8,
    inherit.aes = FALSE
  ) + 
  scale_color_manual(values = cols[3:4]) + # fill of point-locations
  scale_shape_manual(values = chars) + # shape of point-locations
...
values(pvalprop_reclass) <- cut(values(pvalprop_raster), c(-Inf, p_thresh, Inf))
...
ggmap(base_map) + # basemap 
  ...
  geom_spatraster(
    data = pvalprop_reclass, # output SpatRaster
    size = 0,
    alpha = 0.5
  ) +
  scale_fill_manual(
